# EcosystemRealty 2.0 — Master Frontend & System Encyclopedia

> **The Definitive Single-File Master Reference**: This comprehensive document consolidates **all frontend audits, architecture specifications, design systems, component strategies, screen redesign blueprints, pitch playbooks, and marketing website plans** created for EcosystemRealty 2.0.
>
> **Project**: EcosystemRealty 2.0 (Real Estate Sales & Intelligence Operating System)  
> **Market Focus**: Luxury Indian Residential Real Estate (Delhi NCR, Mumbai MMR, Bengaluru, Pune, Hyderabad)  
> **Backend & Platform Status**: 239 Vitest Tests Passing (28 Suites), 21 DB Migrations, 63+ Authenticated API Endpoints, 34 RLS Tables, 75 Compiled Next.js Routes.
...
- [Part VI: Scorecard & Master Execution Roadmap](#part-vi-scorecard--master-execution-roadmap)
  - [30. EcosystemRealty UX Scorecard (0–10 Rating across 15 Dimensions)](#30-ecosystemrealty-ux-scorecard)
...
| :--- | :--- |
| **Product Name** | **EcosystemRealty 2.0** (Luxury Indian Real Estate Sales & Intelligence Operating System) |
...
## 30. EcosystemRealty UX Scorecard
...
# 13. CALLCRM 2.0 ENTERPRISE DOMAIN LAYER & N8N INTEGRATION ARCHITECTURE

## 13.1 Core Architecture Axiom
> **"EcosystemRealty owns the business truth. n8n moves information and triggers actions around that truth."**

EcosystemRealty operates as a 100% self-contained application using Next.js 15 App Router, Supabase PostgreSQL, and PostgreSQL Row-Level Security. It does **not** depend on n8n for any core transactional functionality.

## 13.2 Enterprise Domain Hierarchy & Database Schema (Migration 0020 & 0021)
1. **`people` Registry**: Decoupled from leads, enabling a single person (*Rahul Sharma*) to have multiple active requirements, own units, and refer buyers without contact duplication.
2. **`buyer_requirements`**: Multi-criteria requirements engine (min/max budgets, floor preferences, Vaastu compliance, parking).
3. **`property_listings` & `seller_mandates`**: Exclusivity windows, key custodians, viewing notice SLAs, and **role-protected seller price floors** (masked from junior reps).
4. **`negotiation_rounds`**: Chronological bidding ledger tracking buyer offers, seller counters, ask price deltas, conditions, and token deposits.
5. **`site_visit_dispatches`**: Operational dispatch tracking with digital Gate 2 visitor pass PINs, visitor parking bays, caretaker contacts, and 5-point pre-visit checklists.
6. **`commission_ledgers`**: Statutory Indian brokerage tracking with 1% buyer + 1% seller splits, 18% GST addition, 1% TDS deduction (u/s 194H), channel partner shares, and sales rep incentives.
7. **`crm_domain_events` (Transactional Outbox)**: Persists emitted business events with HMAC-SHA256 signatures, 3-second timeouts, and automatic circuit breakers tripping after 5 consecutive failures.
8. **`inbound_integration_events`**: Idempotent audit log ensuring external webhooks (Meta, WhatsApp, n8n) never create duplicate records on repeated deliveries.

## 13.3 AI Safety Protocol
- AI is an assistant, not an autonomous mutation engine.
- All AI extraction suggestions from n8n are flagged `requires_human_approval: true` and cannot modify deal stages, prices, or commission ledgers without explicit human operator confirmation.
