...
import {
  ...
  INITIAL_MATERIALS_REGIONS,
  INITIAL_MATERIALS_USERS,
  INITIAL_MATERIALS_PROJECTS,
  INITIAL_MATERIALS_UNITS,
  INITIAL_MATERIALS_PEOPLE,
  INITIAL_MATERIALS_LEADS,
  INITIAL_MATERIALS_ACTIVITIES,
  INITIAL_MATERIALS_TASKS,
} from "../lib/mock-data";
...
export function CRMProvider({
  ...
  defaultVertical?: EcosystemVertical;
}) {
  const { user: authUser, org: authOrg, isLoading: authLoading } = useAuth();

  const vertical: EcosystemVertical = authOrg?.industry || defaultVertical || "real_estate";
  ...
  const isMaterials = vertical === "building_materials";

  const [regions, setRegions] = React.useState<Region[]>(isMaterials ? INITIAL_MATERIALS_REGIONS : INITIAL_REGIONS);
  const [areas, setAreas] = React.useState<PropertyArea[]>(isMaterials ? [] : INITIAL_AREAS);
  const [users, setUsers] = React.useState<User[]>(isMaterials ? INITIAL_MATERIALS_USERS : INITIAL_USERS);
  const [projects, setProjects] = React.useState<Project[]>(isMaterials ? INITIAL_MATERIALS_PROJECTS : INITIAL_PROJECTS);
  const [towers, setTowers] = React.useState<ProjectTower[]>(isMaterials ? [] : INITIAL_TOWERS);
  const [units, setUnits] = React.useState<ProjectUnit[]>(isMaterials ? INITIAL_MATERIALS_UNITS : INITIAL_UNITS);
  const [people, setPeople] = React.useState<Person[]>(isMaterials ? INITIAL_MATERIALS_PEOPLE : INITIAL_PEOPLE);
  const [externalOrgs, setExternalOrgs] = React.useState<ExternalOrganization[]>(isMaterials ? [] : INITIAL_EXTERNAL_ORGS);
  const [relationships, setRelationships] = React.useState<EntityRelationship[]>(isMaterials ? [] : INITIAL_RELATIONSHIPS);
  const [propertyFacts, setPropertyFacts] = React.useState<PropertyFact[]>(isMaterials ? [] : INITIAL_PROPERTY_FACTS);
  const [priceHistories, setPriceHistories] = React.useState<UnitPriceHistory[]>(isMaterials ? [] : INITIAL_PRICE_HISTORY);
  const [leads, setLeads] = React.useState<Lead[]>(isMaterials ? INITIAL_MATERIALS_LEADS : INITIAL_LEADS);
  const [activities, setActivities] = React.useState<Activity[]>(isMaterials ? INITIAL_MATERIALS_ACTIVITIES : INITIAL_ACTIVITIES);
  const [tasks, setTasks] = React.useState<Task[]>(isMaterials ? INITIAL_MATERIALS_TASKS : INITIAL_TASKS);
  const [documents, setDocuments] = React.useState<CRMDocument[]>(isMaterials ? [] : INITIAL_DOCUMENTS);
  const [sellerOpportunities, setSellerOpportunities] = React.useState<SellerOpportunity[]>(isMaterials ? [] : INITIAL_SELLER_OPPORTUNITIES);
  const [siteVisitBriefings, setSiteVisitBriefings] = React.useState<SiteVisitBriefing[]>(isMaterials ? [] : INITIAL_SITE_VISIT_BRIEFINGS);

  // Synchronize initial datasets when the vertical/industry changes
  React.useEffect(() => {
    if (vertical === "building_materials") {
      setRegions(INITIAL_MATERIALS_REGIONS);
      setAreas([]);
      setUsers(INITIAL_MATERIALS_USERS);
      setProjects(INITIAL_MATERIALS_PROJECTS);
      setTowers([]);
      setUnits(INITIAL_MATERIALS_UNITS);
      setPeople(INITIAL_MATERIALS_PEOPLE);
      setExternalOrgs([]);
      setRelationships([]);
      setPropertyFacts([]);
      setPriceHistories([]);
      setLeads(INITIAL_MATERIALS_LEADS);
      setActivities(INITIAL_MATERIALS_ACTIVITIES);
      setTasks(INITIAL_MATERIALS_TASKS);
      setDocuments([]);
      setSellerOpportunities([]);
      setSiteVisitBriefings([]);
    } else {
      ...
      setExternalOrgs(INITIAL_EXTERNAL_ORGS);
      setRelationships(INITIAL_RELATIONSHIPS);
      setPropertyFacts(INITIAL_PROPERTY_FACTS);
      setPriceHistories(INITIAL_PRICE_HISTORY);
      setLeads(INITIAL_LEADS);
      setActivities(INITIAL_ACTIVITIES);
      setTasks(INITIAL_TASKS);
      setDocuments(INITIAL_DOCUMENTS);
      setSellerOpportunities(INITIAL_SELLER_OPPORTUNITIES);
      setSiteVisitBriefings(INITIAL_SITE_VISIT_BRIEFINGS);
    }
  }, [vertical]);
  ...
  React.useEffect(() => {
    const saved = localStorage.getItem("ecosystemrealty_active_user_id");
  ...
  const switchActiveUser = React.useCallback((userId: string) => {
    if (userId === "auth" || !userId) {
      ...
      if (typeof window !== "undefined") {
        localStorage.removeItem("ecosystemrealty_active_user_id");
      }
    } else {
      setActiveUserId(userId);
      if (typeof window !== "undefined") {
        localStorage.setItem("ecosystemrealty_active_user_id", userId);
      }
    }
  }, []);
  ...
  const currentUser = React.useMemo<User>(() => {
    ...
    return users[0] || (isMaterials ? INITIAL_MATERIALS_USERS[0] : INITIAL_USERS[0]);
  }, [activeUserId, users, authUser, currentOrgId, isMaterials]);
      ...
      if (!existingPerson) {
        setPeople((prev) => {
          const newPerson: Person = {
            id: linkedPersonId,
            orgId: newLead.orgId,
            fullName: newLead.personName,
            name: newLead.personName,
            primaryPhone: newLead.phone,
            phone: newLead.phone,
