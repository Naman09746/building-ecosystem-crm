...
interface AuthContextType {
  ...
  // Actions
  signUp: (email: string, password: string, name: string, industry?: EcosystemVertical) => Promise<{ success: boolean; error?: string; nextPath?: string }>;
  signIn: (email: string, password: string) => Promise<{ success: boolean; error?: string; nextPath?: string }>;
  signInWithGoogle: () => Promise<{ success: boolean; error?: string }>;
  signInAsDemo: (role?: "owner" | "manager" | "salesperson", industry?: EcosystemVertical) => void;
...
const STORAGE_KEYS = {
  ORG: "ecosystemrealty_auth_org",
  STEP: "ecosystemrealty_workflow_step",
  ONBOARDING: "ecosystemrealty_onboarding_data",
  DEMO_ROLE: "ecosystemrealty_demo_role",
} as const;
    ...
    if (profile) {
      ...
      if (orgData) {
        ...
        authOrg = {
          ...
          complexityMode: orgData.complexity_mode || "deep",
          setupCompletedAt,
...
export function AuthProvider({
  ...
  defaultIndustry?: EcosystemVertical;
}) {
    ...
    async function initAuth() {
      ...
      try {
        ...
        if (!hasAuthenticatedSession) {
          const demoRole = localStorage.getItem(STORAGE_KEYS.DEMO_ROLE) as "owner" | "manager" | "salesperson" | null;
          const demoIndustry = (localStorage.getItem("ecosystemrealty_demo_industry") as EcosystemVertical) || defaultIndustry || "real_estate";
          if (demoRole) {
            signInAsDemo(demoRole, demoIndustry);
          }
        }
      } catch (e) {
  ...
  const signUp = async (
    ...
    password: string, 
    name: string, 
    industry: EcosystemVertical = "real_estate"
  ) => {
    const supabase = getSupabaseClient();
    ...
    try {
      try {
        localStorage.setItem("ecosystemrealty_signup_industry", industry);
      } catch {}
  ...
  const signInAsDemo = (
    role: "owner" | "manager" | "salesperson" = "salesperson",
    industry: EcosystemVertical = "real_estate"
  ) => {
    const isMaterials = industry === "building_materials";
    const demoUsers: Record<string, AuthUser> = isMaterials
      ? {
          owner: { id: "usr-rajesh", name: "Rajesh Jindal", email: "rajesh@jindalsupplies.in", role: "owner" },
          manager: { id: "usr-sunil", name: "Sunil Aggarwal", email: "sunil@jindalsupplies.in", role: "manager" },
          salesperson: { id: "usr-ramesh", name: "Ramesh Patel", email: "ramesh@jindalsupplies.in", role: "salesperson" },
        }
      : {
    ...
    const demoOrg: AuthOrg = isMaterials
      ? {
          id: "org-jindal-supplies",
          name: "Jindal Materials & Marble Yard",
          slug: "jindal-supplies",
          plan: "growth",
          billingCycle: "monthly",
          primaryRegion: "NCR — Gurgaon, Delhi, Noida",
          teamSize: "6-20",
    ...
    try {
      ...
      localStorage.setItem("ecosystemrealty_demo_industry", industry);
      document.cookie = `ecosystemrealty_demo_session=1; path=/; max-age=86400; SameSite=Lax`;
    } catch {}
  };
  ...
  const signOut = async () => {
    ...
    try {
      ...
      document.cookie = "ecosystemrealty_demo_session=; path=/; max-age=0";
    } catch {}
  };
