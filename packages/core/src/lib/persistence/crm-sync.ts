...
import { mapCanonicalRole } from "../server/rbac";
import type {
  Activity,
  ActivityType,
  CRMDocument,
...
export function leadToRow(lead: Partial<Lead>): AnyRow {
  ...
  set("person_id", lead.personId || null);
...
export function unitToRow(unit: Partial<ProjectUnit>): AnyRow {
  ...
  set("occupancy_status", unit.occupancyStatus);
...
export function mapPersonRow(row: AnyRow): Person {
  return {
    ...
    panNumber: row.pan_number ? str(row.pan_number) : undefined,
    kycStatus: row.kyc_status ?? undefined,
    wealthTier: row.wealth_tier ?? undefined,
    isVip: row.is_vip !== undefined ? Boolean(row.is_vip) : undefined,
...
export async function hydrateCrmData(): Promise<CrmHydration | null> {
  ...
  if (typeof document !== "undefined" && document.cookie.includes("ecosystemrealty_demo_session=1")) {
    return null;
  }
