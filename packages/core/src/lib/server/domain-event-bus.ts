...
/**
 * Emit a Domain Event to the EcosystemRealty Transactional Outbox.
...
export async function emitDomainEvent(input: EmitEventInput): Promise<{ success: boolean; eventId: string }> {
  ...
  const idempotencyKey = input.idempotencyKey || `${input.orgId}_${input.eventName}_${input.aggregateId}`;
  ...
  const eventRecord: DomainEventRecord = {
    ...
    payload: {
      ...
      _meta: {
        ...
        source: "EcosystemRealty_Core_Domain",
      },
    },
    idempotencyKey,
    ...
    for (const event of pendingEvents) {
      for (const endpoint of endpoints) {
        ...
        if (!endpoint.secret_token) {
          console.error(`[DOMAIN_EVENT_BUS] Endpoint ${endpoint.id} missing secret_token; skipping dispatch`);
          failed++;
          continue;
        }
        ...
        const signature = crypto
          .createHmac("sha256", endpoint.secret_token)
          .update(rawBody)
          .digest("hex");
        ...
        try {
          ...
          const response = await fetch(endpoint.endpoint_url, {
            ...
            headers: {
              ...
              "X-EcosystemRealty-Event": event.event_name,
              "X-EcosystemRealty-Event-Id": event.id,
              "X-EcosystemRealty-HMAC-SHA256": `sha256=${signature}`,
              "X-EcosystemRealty-Org-Id": orgId,
            },
            body: rawBody,
