...
export function structureFreeTextMeetingNotes(
  ...
  if (input.vertical === "building_materials") {
    if (text.includes("price") || text.includes("expensive") || text.includes("budget") || text.includes("lakh") || text.includes("high")) {
      objections.push("Price / Wholesale Rate Sensitivity");
    }
    if (text.includes("delivery") || text.includes("freight") || text.includes("transport") || text.includes("truck")) {
      objections.push("Delivery / Logistics Concerns");
    }
    if (text.includes("quality") || text.includes("grade") || text.includes("brand") || text.includes("strength")) {
      objections.push("Material Grade / Quality Concerns");
    }
    if (text.includes("credit") || text.includes("terms") || text.includes("days") || text.includes("payment")) {
      objections.push("Credit Term Mismatch");
    }
  } else {
    // Real Estate default
  ...
  if (input.vertical === "building_materials") {
    if (text.includes("need") || text.includes("urgent") || text.includes("shortage") || text.includes("immediate")) {
      buyingSignals.push("Urgent Site Requirement");
    }
    if (text.includes("order") || text.includes("po") || text.includes("advance") || text.includes("book")) {
      buyingSignals.push("Purchase Order Readiness");
    }
    if (text.includes("project") || text.includes("contractor") || text.includes("builder") || text.includes("site")) {
      buyingSignals.push("Verified Ongoing Project");
    }
  } else {
