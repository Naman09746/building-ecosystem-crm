...
import { z } from "zod";
import { apiSuccess, apiError, checkRateLimit, handleValidationError } from "@/lib/server/api-security";
import { getApiAuthContext, getAuthenticatedServerClient, isLiveSupabaseAvailable } from "@/lib/server/supabase-server";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const normalizedPhone = validated.primaryPhone.replace(/\D/g, "").slice(-10);
