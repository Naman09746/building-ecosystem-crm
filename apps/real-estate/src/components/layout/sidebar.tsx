...
import { isManagerRole, roleLabel } from "@repo/core/lib/rbac";
...
export function Sidebar({
  ...
  const materialsOperationsItems = [
    { id: "footfall", label: "Walk-in Log", href: "/footfall", icon: Footprints },
    { id: "scouting", label: "Site Scouting", href: "/scouting", icon: Camera },
    { id: "khata", label: "Khata Ledger", href: "/khata", icon: IndianRupee },
    { id: "rates", label: "Daily Rates", href: "/rates", icon: TrendingUp },
  ];

  const cementNetworkItems = [
    { id: "dealers", label: "Dealer Network", href: "/dealers", icon: Users },
    { id: "sites", label: "Construction Sites", href: "/sites", icon: Building2 },
    { id: "targets", label: "Targets vs Actual", href: "/targets", icon: ChartNoAxesCombined },
    { id: "collections", label: "Collections", href: "/collections", icon: IndianRupee },
    { id: "complaints", label: "Complaints", href: "/complaints", icon: Shield },
    { id: "schemes", label: "Schemes & Accrual", href: "/schemes", icon: ChartNoAxesCombined },
  ];
    ...
    >
      ...
      <div className="overflow-y-auto overflow-x-hidden">
        ...
        <div className={cn("py-3 space-y-4", collapsed ? "px-2" : "px-3")}>
          ...
          {/* COMMERCIAL OPERATIONS (MATERIALS ONLY) */}
          {vertical === "building_materials" && (
            <div className="space-y-1">
              {!collapsed && (
                <div className="px-2 pb-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
              ...
              {materialsOperationsItems.map((item) => {
                const Icon = item.icon;
                const isActive = activeTab ? activeTab === item.id : pathname === item.href;
              ...
              return (
                <a
                  ...
                  className={cn(
                    "flex items-center rounded-lg text-xs font-medium transition-colors cursor-pointer min-h-[36px]",
                    collapsed ? "justify-center p-2" : "justify-between px-2.5 py-1.5",
                    isActive ? "bg-secondary text-foreground font-semibold shadow-subtle" : "text-muted-foreground hover:bg-secondary/60 hover:text-foreground"
                  )}
                >
                  <div className="flex items-center gap-2.5">
                    <Icon className="h-4 w-4 shrink-0 stroke-[1.75]" />
          ...
          {/* TIER 2: PROPERTY INTELLIGENCE / INVENTORY */}
          <div className="space-y-1">
            {!collapsed && (
              <div className="px-2 pb-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                {vertical === "building_materials" ? "Inventory & Catalog" : "Property Intelligence"}
              </div>
            )}
