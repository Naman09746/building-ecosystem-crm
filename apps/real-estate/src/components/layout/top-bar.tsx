...
export function TopBar({ onOpenQuickLog, onOpenSearch, onToggleMobileMenu }: TopBarProps) {
  const { currentUser, users, switchActiveUser, vertical } = useCRM();
  const { user: authUser, org, signOut, resetWorkflow, updateEcosystemSettings } = useAuth();
  const [profileOpen, setProfileOpen] = React.useState(false);

  const currentVertical = org?.industry || vertical || "real_estate";
  ...
  return (
    <header className="h-14 border-b border-border bg-card/95 backdrop-blur-sm px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30 shadow-subtle">
      ...
      {/* Right Side: Vertical Switcher, Quick Action, Notifications & User Profile */}
      <div className="flex items-center gap-2.5 sm:gap-3">
        {/* Industry Vertical Indicator & Switcher */}
        <button
          type="button"
          onClick={() => {
            const nextVertical = currentVertical === "building_materials" ? "real_estate" : "building_materials";
            updateEcosystemSettings(nextVertical, org?.complexityMode || "deep");
          }}
          className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-lg border border-border bg-secondary/50 hover:bg-secondary transition-all hover-lift"
          title="Click to toggle between Real Estate & Building Materials workspaces"
        >
          {currentVertical === "building_materials" ? (
            <>
              <span className="text-emerald-600 font-bold flex items-center gap-1">
                <span>🧱</span> Materials Mode
              </span>
              <span className="text-[10px] text-muted-foreground bg-card border border-border px-1.5 py-0.2 rounded font-mono hidden sm:inline">
                ⇄ Real Estate
              </span>
            </>
          ) : (
