...
import { emitDomainEvent } from "@repo/core/lib/server/domain-event-bus";
...
export async function POST(req: NextRequest) {
  ...
  const authHeader = req.headers.get("Authorization") || req.headers.get("X-EcosystemRealty-Service-Key");
  const expectedSecret = process.env.N8N_SERVICE_SECRET || process.env.NEXTAUTH_SECRET;
  if (!expectedSecret) {
    return apiError("Integration service unavailable: secret unconfigured", 503, "SERVICE_UNAVAILABLE");
  }
  ...
  try {
    ...
    if (!supabase || !isLiveSupabaseAvailable) {
      return apiSuccess({
        ...
        message: "Payload accepted by EcosystemRealty",
      }, 200);
    }
