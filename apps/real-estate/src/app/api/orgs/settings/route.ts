...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data: org, error } = await supabase
      .from("orgs")
      .select("id, name, slug, plan, billing_cycle, trial_ends_at, setup_completed_at, team_size, primary_region, industry, complexity_mode, max_leads, max_seats, reactivation_days, custom_settings, created_at")
      .eq("id", auth.orgId)
      .single();
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (validated.complexityMode !== undefined) updatePayload.complexity_mode = validated.complexityMode;
